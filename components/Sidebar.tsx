...
const Sidebar: React.FC<SidebarProps> = ({ currentSection, setSection }) => {
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950/80 backdrop-blur-2xl border-b border-white/5 z-[250] flex items-center justify-between px-6 shadow-2xl">
        ...
        <Avatar size="sm" className="ring-2 ring-blue-500/20" onClick={() => setSection(AppSection.HOME)} />
      </div>

      {/* Navegação Principal */}
      <nav className="
        fixed bottom-0 left-0 right-0 h-[76px]
        bg-slate-950/90 backdrop-blur-3xl border-t border-white/5 z-[250] 
      ...
      ">
        ...
        <div className="hidden md:flex flex-col items-center gap-6 mb-10 px-4 text-center w-full">
          ...
          <Avatar size="xl" className="ring-4 ring-slate-800/30" onClick={() => setSection(AppSection.HOME)} />
        </div>
            ...
            return (
              <button
                ...
                className={`
                  ...
                  min-w-[70px] md:min-w-0 h-full snap-center
              ...
              >
                <div className={`
                  ...
                  transition-all duration-300 ${isActive ? '-translate-y-1 md:translate-y-0' : ''}
                `}>
                  <item.icon 
                    size={isActive ? 22 : 20} 
                ...
                {isActive && (
                  <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-blue-500 rounded-full md:hidden"></div>
                )}
              </button>
            );
          })}
